...
is_valid_expression_in_aggregation <- function(expr, group_by, var_names = all_cols(expr), agg = FALSE) {
  if(deparse(expr) %in% group_by) {
    return(TRUE)
    ...
    out <- lapply(expr, is_valid_expression_in_aggregation, group_by, var_names, agg)
  }
...
is_aggregate_function_call <- function(expr) {
  identical(typeof(expr), "language") && length(expr) > 1 &&
    (deparse(expr[[1]]) %in% r_aggregate_functions ||
       (expr[[1]] == "paste" && "collapse" %in% names(expr) &&
          !is.null(expr[[which(names(expr) == "collapse")]])))
}

# example usage:
#
#group_by <- deparse(quote(Species))
#good <- quote(1 + mean(Sepal.Length + 3))
#bad <- quote((Sepal.Length + 3) - mean(Sepal.Length + 3))
#
#is_aggregate_expression(good)                      # returns TRUE
#is_valid_expression_in_aggregation(good, group_by) # returns TRUE
#
#is_aggregate_expression(good)                      # returns TRUE
#is_valid_expression_in_aggregation(bad, group_by)  # returns FALSE
#
